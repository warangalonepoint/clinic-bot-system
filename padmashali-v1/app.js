...
function setTheme(mode){ if(mode==='light'){root.classList.add('light'); sun?.classList.add('hidden'); moon?.classList.remove('hidden');} else {root.classList.remove('light'); sun?.classList.remove('hidden'); moon?.classList.add('hidden');} localStorage.setItem('pdm-theme',mode); }
...
roleTabs.forEach(b=> b.addEventListener('click', ()=>{
  roleTabs.forEach(x=>x.classList.remove('active')); b.classList.add('active'); roleSel=b.dataset.role;
}));
...
function logout(){ localStorage.removeItem('pdm-role'); activeRole=null; app.style.display='none'; tabbar.style.display='none'; loginSec.style.display=''; loginMsg.textContent=''; pinInput.value=''; }
...
function goTab(id){
  ...
  // small UX: keep selected tab visible in scroller
  const el = document.querySelector(`.tab-btn[data-target="${id}"]`); el?.scrollIntoView({inline:'center', behavior:'smooth', block:'nearest'});
}
window.goTab = goTab;
...
function renderNews(){ if(!newsList) return; newsList.innerHTML=''; news.forEach(n=>{ const li=document.createElement('div'); li.className='list-item glass-lite'; li.innerHTML=`<div><div class="title">${n.title}</div><div class="sub">${n.tag}</div><p class="muted" style="margin:6px 0 0">${n.body}</p></div><div><a class="chip" target="_blank" href="https://wa.me/919999999999?text=${encodeURIComponent(n.title)}">Share</a></div>`; newsList.appendChild(li); }); }
renderNews();

// directory
const dirList=document.getElementById('dirList'), dirQuery=document.getElementById('dirQuery'), dirRole=document.getElementById('dirRole');
function renderDir(){ if(!dirList) return; const term=(dirQuery?.value||'').toLowerCase(), role=dirRole?.value||''; dirList.innerHTML=''; members.filter(m=>(!role||m.role===role)&&(m.name.toLowerCase().includes(term)||m.role.toLowerCase().includes(term)||m.city.toLowerCase().includes(term))).forEach(m=>{ const li=document.createElement('div'); li.className='list-item glass-lite'; li.innerHTML=`<div><div class="title">${m.name}</div><div class="sub">${m.role} • ${m.city}</div></div><div class="row"><a class="chip" href="tel:${m.phone}">Call</a><a class="chip" target="_blank" href="https://wa.me/${m.phone}?text=${encodeURIComponent('Jai Padmashali!')}">WhatsApp</a></div>`; dirList.appendChild(li); }); }
renderDir(); dirQuery?.addEventListener('input',renderDir); dirRole?.addEventListener('change',renderDir);

// events
const eventList=document.getElementById('eventList');
function renderEvents(){ if(!eventList) return; eventList.innerHTML=''; events.forEach(ev=>{ const li=document.createElement('div'); li.className='list-item glass-lite'; const d=new Date(ev.date).toDateString(); li.innerHTML=`<div><div class="title">${ev.title}</div><div class="sub">${d} • ${ev.time} • ${ev.place}</div></div><div class="row"><a class="chip" href="tel:${ev.contact}">Call</a><a class="chip" target="_blank" href="https://wa.me/${ev.contact}?text=${encodeURIComponent('I want to join: '+ev.title)}">Join</a></div>`; eventList.appendChild(li); }); }
renderEvents();
...
document.getElementById('hallBook')?.addEventListener('click',()=>{
  const hall=document.getElementById('hallSel').value, date=document.getElementById('hallDate').value, time=document.getElementById('hallTime').value, purpose=document.getElementById('hallPurpose').value||'—';
  ...
  window.open(`https://wa.me/919999999999?text=${msg}`,'_blank'); hallMsg.textContent=(curLang==='te'?'వాట్సాప్‌కి పంపబడింది':'Request sent via WhatsApp');
});
...
// loan
const lrName=document.getElementById('lrName'), lrPhone=document.getElementById('lrPhone'), lrAmt=document.getElementById('lrAmt'), lrPurpose=document.getElementById('lrPurpose'), lrNotes=document.getElementById('lrNotes'), lrMsg=document.getElementById('lrMsg'), lrWA=document.getElementById('lrWA');
function loanText(){ return `Loan/Help Request%0AName: ${encodeURIComponent(lrName.value||'')}`+`%0APhone: ${encodeURIComponent(lrPhone.value||'')}`+`%0AAmount: ₹${encodeURIComponent(lrAmt.value||'')}`+`%0APurpose: ${encodeURIComponent(lrPurpose.value||'')}`+`%0ANotes: ${encodeURIComponent(lrNotes.value||'')}`; }
document.getElementById('lrSubmit')?.addEventListener('click',()=>{ if(!(lrName.value&&lrPhone.value&&lrAmt.value)){ lrMsg.textContent=(curLang==='te'?'పేరు, ఫోన్, మొత్తం తప్పనిసరి':'Name, Phone, Amount are required'); return; } lrMsg.textContent=(curLang==='te'?'స్థానికంగా సేవ్‌ైంది.':'Saved locally.'); lrWA.setAttribute('href',`https://wa.me/919999999999?text=${loanText()}`); });
...
const I18N={
  ...
  te:{'app.title':'పద్మశాలి కమ్యూనిటీ','app.tagline':'ఐక్యత • సేవ • అభివృద్ధి',
      ...
      'home.news':'సంస్థ వార్తలు','home.quick':'త్వరిత లింకులు','home.whatsapp':'వాట్సాప్ గ్రూప్','home.upcoming':'రాబోయే కార్య‌క్ర‌మాలు','home.pay':'సభ్యత్వ చెల్లింపు',
...
function applyI18N(){
  ...
  // selects
  const hallSel=document.getElementById('hallSel'); if(hallSel){ hallSel.options[0].text=t['_opt.h1']; hallSel.options[1].text=t['_opt.h2']; }
  const hallTime=document.getElementById('hallTime'); if(hallTime){ hallTime.options[0].text=t['_opt.m']; hallTime.options[1].text=t['_opt.a']; hallTime.options[2].text=t['_opt.e']; hallTime.options[3].text=t['_opt.f']; }
  // toggle label
  const langBtn=document.getElementById('langBtn'); if(langBtn) langBtn.textContent=(curLang==='te')?'English':'తెలుగు';
}
let curLang = localStorage.getItem('pdm-lang') || 'en';
applyI18N();
document.getElementById('langBtn')?.addEventListener('click',()=>{ curLang=(curLang==='en')?'te':'en'; localStorage.setItem('pdm-lang',curLang); applyI18N(); });
